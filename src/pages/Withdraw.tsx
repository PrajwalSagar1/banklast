...
import { Wallet, ArrowUpFromLine, Info, CheckCircle, ChevronDown, Upload } from 'lucide-react';
...
const Withdraw = () => {
  ...
  useEffect(() => {
    api.get('/transactions/withdrawals').then((r) => setRecent(r.data.withdrawals ?? [])).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/account/withdraw', { amount: Number(amount), paymentMethod: method, description });
      setBalance(data.newBalance);
      setSuccess(`Withdrawal of ${fmt(Number(amount))} successful!`);
      setAmount(''); setMethod(''); setDescription('');
      const r = await api.get('/transactions/withdrawals');
      setRecent(r.data.withdrawals ?? []);
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-xl p-6" style={{ backgroundColor: '#0d1526', border: '1px solid #1e293b' }}>
          <h3 className="text-base font-semibold text-white mb-1">Make a Withdrawal</h3>
          <p className="text-sm mb-6" style={{ color: '#94a3b8' }}>Withdraw money from your account securely.</p>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button type="submit" disabled={loading}
              ...
              <Upload size={16} /> {loading ? 'Processing...' : 'Withdraw Now'}
            </button>
          </form>
        </div>
        ...
        <div className="space-y-5">
          <div className="rounded-xl p-5" style={{ backgroundColor: '#0d1526', border: '1px solid #1e293b' }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-base font-semibold text-white">Withdrawal Information</h3>
            </div>
            <ul className="space-y-3">
              {['Instant processing', 'Secure & encrypted transactions', '24/7 availability', 'No hidden charges'].map((item) => (
          ...
          <div className="rounded-xl p-5" style={{ backgroundColor: '#0d1526', border: '1px solid #1e293b' }}>
            ...
            {recent.length === 0 ? (
              <p className="text-sm text-center py-4" style={{ color: '#475569' }}>No withdrawals yet.</p>
            ) : (
              <ul className="space-y-3">
                {recent.map((d) => (
                  <li key={d._id} className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-9 h-9 rounded-full flex-shrink-0" style={{ backgroundColor: 'rgba(239,68,68,0.15)' }}>
                    ...
                    <span className="text-sm font-semibold" style={{ color: '#ef4444' }}>-{fmt(d.amount)}</span>
                  </li>
                ))}
              </ul>
            )}
